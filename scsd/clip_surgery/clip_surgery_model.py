...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if side != new_h or side != new_w:
            new_pos = self.positional_embedding[1:, :].reshape(-1, side, side, x.shape[-1]).permute(0, 3, 1, 2)
            new_pos = torch.nn.functional.interpolate(new_pos, (new_h, new_w), mode='bilinear')
            new_pos = new_pos.reshape(-1, x.shape[-1], new_h * new_w).transpose(1, 2)
            positional_embedding = torch.cat([self.positional_embedding[:1, :], new_pos[0]], 0)

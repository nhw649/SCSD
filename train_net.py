...
class Trainer(DefaultTrainer):
    ...
    def build_optimizer(cls, cfg, model):
        ...
        name_param_dict = {name: param for name, param in model.named_parameters()}
        for i, param_group in enumerate(optimizer.param_groups):
            print(f"Param Group {i}:")
            for param in param_group['params']:
                for name, p in name_param_dict.items():
                    if p is param:
...
def setup(args):
    ...
    setup_logger(output=cfg.OUTPUT_DIR, distributed_rank=comm.get_rank(), name="SCSD")
    return cfg
